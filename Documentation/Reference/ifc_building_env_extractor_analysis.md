...
## Роль TU Delft в проекте

- TU Delft IfcEnvelopeExtractor используется как **внешний движок**.
- Управление выполняется через JSON-конфиг и запуск exe.
- В проект **не встраивается как библиотека** и не требует прямой зависимости в коде.

## Схема пайплайна

IFC → TU Delft EXE → OBJ → trimesh → MeshOptimizer → STL
...
## План сверки с исходным проектом
